...
email_conf_path = "conf/RMS_Weekly_Kodi_Email_Conf.json"
...
# Job Description, PO Number, Customer, CIC+Quant, PO Due Date, Status, Priority, Total Item Quantity, Address, Invoiced Complete
po_filter_properties = [r"%3Bbpx", r"title", r"wxy_", r"DLvO", r"%3CvR%3B", r"hp%5C%3E", r"%5B%7DLB", r"H%5BTO", r"%5E%3Dgd", r"ERGt"] 
...
with open(csv_file[0], mode='w', newline='') as file:
    ...
    csv_writer.writerow(["PO Number", "Description", "CIC+Quantity", "PO Due Date", "Status", "Priority", "Total Item Quantity", "Address"])
    for order in output_dict:
        status_str = ", ".join(output_dict[order]["Status"])
        address_str = " ".join(output_dict[order]["Address"])
        csv_writer.writerow([order, output_dict[order]["Job Description"], output_dict[order]["CIC+Quant"], output_dict[order]["PO Due Date"], status_str, output_dict[order]["Priority"], output_dict[order]["Total Item Quantity"], address_str])
